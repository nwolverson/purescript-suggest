...
getSuggestions warnings = do
  ...
  where
    ...
    getReplacement { source: Just s, error: { suggestion: Just { replacement }, filename: Just filename }, position: Just position  } =
      Just {
        filename,
        position,
        original: joinWith "\n" s,
...
replaceFile lines filename group = do
  ...
  let replaced = replaceFile' 1 lines' group'
  case replaced of
    Left err -> error err
    Right lines -> do
...
replaceFile' _ _ _ = Left "Found replacement after end of file"
